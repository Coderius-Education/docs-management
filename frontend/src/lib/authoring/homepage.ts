...
export function insertSectionInParent(
  source: string,
  parent: LessonNode,
  name: string,
) {
  ...
  const target = {
    from: original.from + prefixSize,
    to: original.to + prefixSize,
  };
  ...
  if (close >= 0)
    return replaceRange(
      without,
      { from: target.from + close, to: target.from + close },
      `\n\n${snippet}\n\n`,
    );
  if (/\/>\s*$/.test(raw))
    return replaceRange(
...
/** MDX may wrap adjacent inline JSX sections in an otherwise empty paragraph. */
export function homepageChildren(node: LessonNode): LessonNode[] {
  return (node.children ?? [])
    .filter((n) => n.type !== 'mdxjsEsm')
    .flatMap((n) =>
      n.type === 'paragraph' &&
      n.children?.some((c) => c.type === 'mdxJsxTextElement') &&
      n.children.every(
        (c) =>
