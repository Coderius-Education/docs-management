...
const colors =
  /^--ifm-(?:color-primary(?:-(?:dark|darker|darkest|light|lighter|lightest))?|background-color|font-color-base)$/;
...
function deriveShades(tokens) {
  ...
  if (typeof primary !== 'string' || !/^#(?:[\da-fA-F]{3}|[\da-fA-F]{6})$/.test(primary))
    return result;
  const hex =
    primary.length === 4 ? [...primary.slice(1)].map((c) => c + c).join('') : primary.slice(1);
  const rgb = [0, 2, 4].map((offset) => Number.parseInt(hex.slice(offset, offset + 2), 16));
  for (const [suffix, factor] of Object.entries({
    dark: 0.9,
    darker: 0.85,
    darkest: 0.7,
    light: 1.1,
    ...
    if (!(name in result))
      result[name] = `#${rgb
        .map((value) =>
          Math.min(255, Math.floor(value * factor + 0.5))
            .toString(16)
            .padStart(2, '0'),
        )
        .join('')}`;
...
function renderCss(tokens = {}) {
  if (!isObject(tokens) || Object.keys(tokens).some((key) => !['light', 'dark'].includes(key)))
    throw new Error('Invalid token modes');
  ...
  for (const mode of ['light', 'dark']) {
    ...
    const declarations = Object.entries(deriveShades(values))
      .sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0))
      .map(([name, value]) => {
        if (typeof value !== 'string' || value.length > 300)
          throw new Error(`Invalid token ${name}`);
        const valid = colors.test(name)
          ? /^(?:#[\da-fA-F]{3,4}|#[\da-fA-F]{6}|#[\da-fA-F]{8}|(?:rgb|rgba|hsl|hsla)\([\d.,% +\-/]+\))$/.test(
              value,
            )
          : fonts.has(name)
...
function validate(settings) {
  if (!isObject(settings) || settings.version !== 1)
    throw new Error('Unsupported site-settings version');
  for (const key of Object.keys(settings))
    if (!['version', 'site', 'themeConfig', 'tokens', 'docs'].includes(key))
      throw new Error(`Unknown settings group ${key}`);
  for (const key of ['site', 'themeConfig', 'tokens', 'docs'])
    if (!isObject(settings[key])) throw new Error(`Invalid ${key} settings`);
  const allowedSite = new Set(['title', 'tagline', 'description', 'keywords', 'favicon', 'image']);
  for (const key of Object.keys(settings.site))
    if (!allowedSite.has(key)) throw new Error(`Unsupported site property ${key}`);
  ...
  const walk = (v) => {
    if (Array.isArray(v)) v.forEach(walk);
      ...
      for (const [k, x] of Object.entries(v)) {
        if (FORBIDDEN.has(k)) throw new Error(`Unsafe settings key ${k}`);
        walk(x);
      }
  };
  walk(settings);
...
function applySettings(config, raw, siteDir) {
  ...
  result.presets = (config.presets || []).map((entry) => {
    ...
    opts.theme = {
      ...opts.theme,
      customCss: [
        ...css.filter((p) => p !== path.join(siteDir, 'src/css/managed-theme.css')),
        path.join(siteDir, 'src/css/managed-theme.css'),
      ],
    };
    return [entry[0], opts];
  });
